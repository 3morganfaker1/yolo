	...
	<div class="system-role-container layout-padding">
		<div class="system-role-padding layout-padding-auto layout-padding-view">
			...
			<el-table :data="state.tableData.data" style="width: 100%">
				...
				<el-table-column label="AI建议" min-width="220" align="center">
					<template #default="scope">
						<div class="suggestion-cell">
							<span class="suggestion-preview">{{ formatSuggestionPreview(scope.row.suggestion) }}</span>
							<el-button
								v-if="scope.row.suggestion && scope.row.suggestion !== '未选择AI，无AI分析！'"
								size="small"
								text
								type="primary"
								@click="openSuggestionDialog(scope.row.suggestion)"
							>
								查看详情
							</el-button>
						</div>
			...
			<el-dialog v-model="state.suggestionDialogVisible" title="AI建议详情" width="760px">
				<div class="suggestion-detail">{{ state.currentSuggestion || '暂无内容' }}</div>
			</el-dialog>
		</div>
	</div>
</template>
		...
		.then((res) => {
			if (res.code == 0) {
				...
				for (let i = 0; i < res.data.records.length; i++) {
					...
					// Backend field is `lable` in current entity getter; keep compatibility with both keys.
					const labels = JSON.parse(record.label || record.lable || '[]');
...
const formatSuggestionPreview = (suggestion: string) => {
	if (!suggestion) return '-';
	const clean = suggestion.replace(/\s+/g, ' ').trim();
	if (clean === '未选择AI，无AI分析！') return clean;
	return clean.length > 26 ? `${clean.slice(0, 26)}...` : clean;
};

const openSuggestionDialog = (suggestion: string) => {
	state.currentSuggestion = suggestion || '';
	state.suggestionDialogVisible = true;
};
