<template>
	<div class="login-container">
		...
		<div class="login-box animate__animated animate__fadeIn">
			<div class="title">
				<h2>基于YOLOv13的学生课堂行为智能检测系统</h2>
				<p>Intelligent Student Classroom Behavior Detection System Based on YOLOv13</p>
...
const onSignIn = async () => {
	...
	try {
		// This project uses frontend-controlled routes in local deployment.
		// Prefer frontend route init to avoid being redirected back to login when backend menu API is unavailable.
		const isNoPower = await initFrontEndControlRoutes();
		signInSuccess(isNoPower);
	} catch (error) {
		console.error('route init failed, fallback to frontend routes', error);
		const isNoPower = await initFrontEndControlRoutes();
		signInSuccess(isNoPower);
	}
};
